...
main(int argc, char** argv)
{
    ...
    static double od[WYMIAR] = { 0 }; 
    static double x_p[WYMIAR] = { 0 };
    double* a=nullptr;
    double t1;
    int n, nt, i, j;
    const int ione = 1;
    ...
    if (size > 1) {
       ...
       for (i = 0; i < WYMIAR; i++) z[i] = 0.0;
        ...
        MPI_Scatter(x_p, n_wier, MPI_DOUBLE, &x[n_wier*rank], n_wier, MPI_DOUBLE, 0, MPI_COMM_WORLD);
   /*     if(rank==0){
        ...
        if (rank == 0) {
            MPI_Gather(MPI_IN_PLACE, n_wier, MPI_DOUBLE, od, n_wier, MPI_DOUBLE, 0, MPI_COMM_WORLD);
        }
        else {
       ...
       // MPI_Gather(&z, n_wier, MPI_DOUBLE, od, n_wier, MPI_DOUBLE, 0, MPI_COMM_WORLD);
        if (rank == 0) {
            printf("Sprawdzamy poprawnosc obliczen \n");
          /*  for (i = 0; i < WYMIAR; i++) {
      ...
      /*      for (i = 0; i < WYMIAR; i++) {
                if (fabs(y[i] - od[i]) > 1.e-9 * od[i]) {
                    printf("tablica odboir: %lf y: %lf z: %lf \n", od[i], y[i], z[i]);
                }
            }
