...
int main(){
  ...
  // 1 przed wydrukami sekcja krytyczna
...
#pragma omp parallel num_threads(10) default(none) shared(a_shared, e_atomic, tmp_a) private(b_private) firstprivate(c_firstprivate)
  {
    f_threadprivate = omp_get_thread_num();

    int i;
    int d_local_private;
    d_local_private = tmp_a + c_firstprivate; //WAR
